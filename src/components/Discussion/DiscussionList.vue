...
export default {
  ...
  setup() {
    ...
    const getDisscussionData = () => {
      loading.value = true;
      axios
        .get("http://175.178.221.165:8081/discussions/getDiscussions", {
          params: {
        ...
        .then((res) => {
          ...
          if (res.data.success === true) {
            discussion_list.value = res.data.data.discussionList.map(
              (item: any) => ({
                id: item.id,
                title: item.title,
            ...
            for (let i = 0; i < discussion_list.value.length; i++) {
              discussion_list.value[i].description = stripHtml(
                discussion_list.value[i].description
              );
            }
    ...
    let onSearch = () => {
      if (inputValue.value === "") {
        ...
        loading.value = true;
        axios
          .get("http://175.178.221.165:8081/discussions/findDiscussions", {
            params: {
