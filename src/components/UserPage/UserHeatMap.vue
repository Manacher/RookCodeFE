<template>
  <div class="heatmap-main">
    <div class="text-area">
      <div>
        过去一年共提交
        <span style="font-size: 1.1rem; color: #262626; font-weight: bold">{{
          totalNum
        }}</span>
        次
      </div>
      ...
      <div style="margin-left: auto; color: #8a8a8e">
        ...
        <a-select
          ...
          style="width: 6.5rem; margin-left: 1.5rem"
        >
    ...
    let init = () => {
      getUserYearSubmit(props.account).then((res: any) => {
        if (res.success) {
          ...
          let lastYearDayString = echarts.format.formatTime(
            "yyyy-MM-dd",
            lastYearDay
          );
